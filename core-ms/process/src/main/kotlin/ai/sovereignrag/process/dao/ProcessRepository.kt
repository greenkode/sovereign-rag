...
import ai.sovereignrag.process.domain.model.ProcessTransitionInfo
...
interface ProcessRepository : JpaRepository<ProcessEntity, Long> {
    ...
    @Cacheable(value = [CacheNames.PROCESS], key = "#publicId.toString() + '_' + #state.name()", unless = "#result == null")
    @Query("""
        SELECT DISTINCT p FROM ProcessEntity p 
        LEFT JOIN FETCH p.requests r 
        LEFT JOIN FETCH r.data
    ...
    fun findByExternalReferenceAndTypeInAndState(externalReference: String, type: Set<ProcessType>, state: ProcessState): ProcessEntity?

    @Cacheable(value = [CacheNames.PROCESS], key = "#externalReference + '_' + #state.name()", unless = "#result == null")
    @Query("""
        SELECT DISTINCT p FROM ProcessEntity p 
        LEFT JOIN FETCH p.requests r 
        LEFT JOIN FETCH r.data
    ...
    @Caching(evict = [
        ...
        CacheEvict(value = [CacheNames.PROCESS], key = "#entity.externalReference + '_' + #entity.state.name()", condition = "#entity.externalReference != null"),
    ...
    @Query("""
        ...
        LEFT JOIN FETCH r.stakeholders
        WHERE p.id = (
            SELECT p2.id FROM ProcessEntity p2
            JOIN p2.requests r2
            JOIN r2.stakeholders s2
            WHERE p2.type = :type
    ...
    fun findActivePendingProcessForTypeAndUser(
        ...
        @Param("userId") userId: String,
        @Param("stakeholderType") stakeholderType: ProcessStakeholderType,
        @Param("inactivityThreshold") inactivityThreshold: Instant
    ): ProcessEntity?
